...
weight_init = tf_contrib.keras.initializers.VarianceScaling() # kaming init for encoder / decoder
...
def resblock(x_init, channels, use_bias=True, scope='resblock'):
    with tf_contrib.variable_scope(scope):
        ...
        with tf_contrib.variable_scope('res2'):
...
def adaptive_avg_pooling(x):
    ...
    gap = tf.math.reduce_mean(x, axis=1, keepdims=True)
...
def adaptive_instance_norm(content, gamma, beta, epsilon=1e-5):
    ...
    c_mean, c_var = tf.nn.moments(content, axes=[1], keepdims=True)
...
def discriminator_loss(type, real, fake):
    ...
    for i in range(n_scale) :
        if type == 'lsgan' :
            real_loss = tf.math.reduce_mean(tf.math.squared_difference(real[i], 1.0))
            fake_loss = tf.math.reduce_mean(tf.math.square(fake[i]))

        if type == 'gan' :
            real_loss = tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(real[i]), logits=real[i]))
            fake_loss = tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(fake[i]), logits=fake[i]))
...
def generator_loss(type, fake):
    ...
    for i in range(n_scale) :
        if type == 'lsgan' :
            fake_loss = tf.math.reduce_mean(tf.math.squared_difference(fake[i], 1.0))

        if type == 'gan' :
            fake_loss = tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(fake[i]), logits=fake[i]))
...
def instance_norm_layer(
    ...
    layer = tfa.layers.InstanceNormalization(
        epsilon=epsilon)
    layer.build(tf.keras.backend.int_shape(inputs))
    instance_norm_layer = layer.call(inputs=inputs)
...
def layer_norm_layer(
    ...
    layer = tf.keras.layers.LayerNormalization(
        ...
        name=name)
    layer.build(tf.keras.backend.int_shape(inputs))
    layer_norm_layer = layer.call(inputs=inputs,)
...
def conv1d_layer(
    ...
    if name.__contains__("discriminator"):
        ...
        weight_init = tf_contrib.keras.initializers.VarianceScaling()

    conv_layer = tf_contrib.layers.conv1d(
        inputs=inputs,
        filters=filters,
...
def conv2d_layer(
    ...
    conv_layer = tf_contrib.layers.conv2d(
        inputs=inputs,
        filters=filters,
        kernel_size=kernel_size,
        strides=strides,
